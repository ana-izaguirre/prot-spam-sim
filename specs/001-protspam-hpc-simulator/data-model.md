...
_structural_ (triangular geometry), not biological. That contrast is the dataset's whole
purpose.
...
**Why it matters**: _Homo sapiens_ at 69.58 Maa against a minimum of 0.58 Maa is a 120×
span, and against the dataset mean it is the source of the thesis' efficiency ceiling
`E_max = mean(n) / max(n)`. Because the key taxa occupy the _lowest_ indices, they also sit
in the _heaviest_ rows of the upper triangle — geometric and biological imbalance stack on
...
strategies differ only in _which rank_ receives that row.
...
_Homo sapiens_) and `balanced` (300 species without the extreme outlier).
...
columns hold the _same_ double, the hex strings are necessarily identical — the panel
